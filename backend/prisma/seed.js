...
const ROLES_FULL = [...ROLE_PLAN_EXPANDED, ...ROLE_PLAN_EXPANDED, ...ROLE_PLAN_EXPANDED];
...
if (NAMES_FULL.length !== ROLES_FULL.length) {
  throw new Error(`seed: NAMES_FULL (${NAMES_FULL.length}) and ROLES_FULL (${ROLES_FULL.length}) must be the same length`);
}
if (new Set(NAMES_FULL).size !== NAMES_FULL.length) {
  throw new Error('seed: NAMES_FULL contains a duplicate full name');
}

// Take only the first 10 of each 50-person shift block — a lighter 30-person roster
// (10 per shift) instead of the full 150, same names/roles/logic otherwise.
const BLOCK_SIZE = 50;
const TAKE_PER_BLOCK = 10;
function sliceBlocks(arr) {
  const out = [];
  for (let b = 0; b < 3; b += 1) out.push(...arr.slice(b * BLOCK_SIZE, b * BLOCK_SIZE + TAKE_PER_BLOCK));
  return out;
}
const NAMES = sliceBlocks(NAMES_FULL);
const ROLES = sliceBlocks(ROLES_FULL);

const SHIFTS_PER_BLOCK = ['Shift 1', 'Shift 2', 'Shift 3'];
function shiftFor(i) {
  return SHIFTS_PER_BLOCK[Math.floor(i / TAKE_PER_BLOCK)];
}
...
async function main() {
  ...
  // email are unique, and this script always writes EMP-0000..EMP-0030. Delete children
  ...
  console.log(`Seed complete: 1 admin + ${employees.length} employees across 3 shifts (10 each).`);
